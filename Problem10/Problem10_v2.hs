...
findAllPrimes_list :: Integer -> [Integer]
findAllPrimes_list target = 
	if(target < 2)
		then []
	else if(isPrime_mod target)
		then [target] ++ (findAllPrimes_list (target - 1))
	else
		(findAllPrimes_list (target - 1))
...
main = 
	do 
		...
		let allPrimes = findAllPrimes_list first
		t1 <- getClockTime
